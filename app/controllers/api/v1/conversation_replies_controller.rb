class Api::V1::ConversationRepliesController < Api::BaseController
  def create
    ...
    u = Urbanairship.push(:aliases => [recipient.alias_name], :aps => { :alert => "#{current_api_user.full_name} said: #{params[:message].truncate(100, :separator => ' ')}", :badge => 1, sound: "default", other: { conversation_id: conversation.id } })
