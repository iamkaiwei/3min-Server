class Api::V1::ChatsController < Api::BaseController
  ...
  def create
    ...
    u = Urbanairship.push(:aliases => [recipient.alias_name], :aps => { :alert => "#{current_api_user.full_name} said: #{params[:message]}", :badge => 1, sound: "default", other: { product_id: product.id } })
    if u.success?
